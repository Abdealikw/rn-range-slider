...
const int THUMBS[] = {THUMB_LOW,THUMB_HIGH};
...
- (void)drawRect:(CGRect)rect {
    ...
    if ([_labelStyle isEqualToString:NONE] || _activeThumb == THUMB_NONE || _alwaysShowLabel == false) {
        return;
    }
    
     for ( int i = 0; i < sizeof(THUMBS); i++ ) {
         
        NSString *text = [self formatLabelText:THUMBS[i] == THUMB_LOW ? _lowValue : _highValue];
        ...
        CGFloat cx = THUMBS[i] == THUMB_LOW ? lowX : highX;
